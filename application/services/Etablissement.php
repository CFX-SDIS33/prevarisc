...
//                 ->setCriteria("d.AVIS_DOSSIER_COMMISSION > 0")
    ...
    public function getDossiers($id_etablissement)
    {
        ...
        $results = array();
        $results['etudes'] = $search->setItem("dossier")->setCriteria("e.ID_ETABLISSEMENT", $id_etablissement)->setCriteria("d.TYPE_DOSSIER", 1)->order("COALESCE(DATECOMM_DOSSIER,DATEINSERT_DOSSIER) DESC")->order("REFCOURRIER_DOSSIER DESC")->run()->getAdapter()->getItems(0, 999999)->toArray();
        $results['visites'] = $search->setItem("dossier")->setCriteria("e.ID_ETABLISSEMENT", $id_etablissement)->setCriteria("d.TYPE_DOSSIER", array(2, 3))->order("COALESCE(DATEVISITE_DOSSIER, DATECOMM_DOSSIER, DATEINSERT_DOSSIER) DESC")->order("REFCOURRIER_DOSSIER DESC")->run()->getAdapter()->getItems(0, 999999)->toArray();
        $results['autres'] = $search->setItem("dossier")->setCriteria("e.ID_ETABLISSEMENT", $id_etablissement)->setCriteria("d.TYPE_DOSSIER", $types_autre)->order("DATEINSERT_DOSSIER DESC")->order("REFCOURRIER_DOSSIER DESC")->run()->getAdapter()->getItems(0, 999999)->toArray();
        return $results;
    }
